...
HTUHub::HTUHub(TwoWire *i2c, uint8_t addr):
    ...
    address(addr),
    sensor(HTU21D()),
    humidity(Sensor<float>("humidity", "HTU", "humidity", new WindowedReading<float, SAMPLE_BACKLOG>("%", 0, 100))),
    temperature(Sensor<float>("temperature", "HTU", "temperature", new WindowedReading<float, SAMPLE_BACKLOG>("°C", -40, 125))),
