...
BMPHub::BMPHub(TwoWire *i2c, uint8_t addr):
    ...
    pressure(Sensor<float>("pressure", "BMP", "pressure", new WindowedValue<float, SAMPLE_BACKLOG>("Pa", 30000, 110000))),
    temperature(Sensor<float>("temperature", "BMP", "temperature", new WindowedValue<float, SAMPLE_BACKLOG>("°C", -40, 85))),
    altitude(Sensor<float>("altitude", "BMP", "altitude", new WindowedValue<float, SAMPLE_BACKLOG>("m", -500, 9000)))
{}
